                ...
                <form class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10" id="exprGrd_div">
                            <input type="text" class="form-control disabled" name="exprGrd" id="exprGrd_input"
                                   autocomplete="off" 
...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="alert alert-info text-center my-alert-nav" role="alert">学 生 实 验 管 理</div>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-md-2">
            <div class="list-group text-center">
                <a href="${APP_PATH}/teacherHomeworkControl/${tId}" class="list-group-item">作业管理</a>
                <a href="${APP_PATH}/teacherExperimentControl/${tId}" class="list-group-item active">实验管理</a>
...
<script>
    ...
    function build_stu_table(result) {
        ...
        $.each(stus, function (index, item) { /*下标和下标对应的信息*/
            ...
            var editBtn = $("<button></button>").addClass("btn btn-info btn-sm").append($("<span></span>")).addClass("glyphicon glyphicon-list-alt edit_btn").append(" 实验");
    ...
    function getStu(id) {
        ...
        $.ajax({
            url: "${APP_PATH}/getExprQuestion/" + id + "/${exprId}",
            type: "get",
            success: function (result) {
                console.log(result);
                var stuData = result.extend.experiment;
                $("#question_input").val(stuData.question === "" ? "尚无实验" : stuData.question);
                $("#answer_input").val(stuData.answer === "" ? "该学生未完成实验" : stuData.answer);
                $("#exprGrd_input").val(stuData.exprGrd === 0 ? "" : stuData.exprGrd);
            }
        })
    }
    ...
    $("#stu_update_btn").click(function () {
        ...
        /*发送ajax请求保存数据*/
        $.ajax({
            url: "${APP_PATH}/updateExprGrade/" + $(this).attr("edit-id") + "/${exprId}",
            type: "put",
